...
//            assertEquals("application/vnd.ms-excel", resource.getMimeType());
    ...
    public void testGetChildren() throws ZebedeeException, IOException {
            Map<URI, ContentNode> children = ZebedeeReader.getInstance().getPublishedContentChildren("peoplepopulationandcommunity/culturalidentity/ethnicity",1, true);
    ...
    public void testGetCollectionChildren() throws ZebedeeException, IOException {
        Map<URI, ContentNode> children = ZebedeeReader.getInstance().getCollectionContentChildren(TEST_COLLECTION_ID, "/employmentandlabourmarket/peopleinwork/workplacedisputesandworkingconditions",1, true);
        assertTrue(children.size() == 2);
